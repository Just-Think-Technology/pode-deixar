...
- **Storage:** SeaweedFS S3 (S3-compatible), bucket `order-photos`
...
- **Proxy:** Caddy applies `strip_prefix /api/storage` on `/api/storage/*`, so
  SeaweedFS S3 (`seaweedfs:8333`) receives the `<bucket>/<object>` path. The presigned client is addressed
  ...
  signature validates on SeaweedFS.
