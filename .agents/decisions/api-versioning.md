...
- **Gateway:** Caddy (`deploy/Caddyfile.docker`, `deploy/Caddyfile.production`) exposes `handle /api/v1/* { reverse_proxy <service> }` for each domain (auth, profiles/providers/categories, services/proposals, payments, reviews) — forwarded unchanged because the Nest prefix is part of the upstream path. Storage (`/api/v1/storage/*`) still strips to `/` since MinIO has no prefix. Health handles `handle /api/v1/<service>/health* { uri strip_prefix /api/v1/<service>; }` strip to `/health` because health is excluded from the Nest prefix, so direct probes (`:3001/health`) still work without the gateway.
- **Legacy:** `handle /api/*` is kept for backward compatibility and rewritten to `/api/v1/*` (`uri replace /api/ /api/v1/ 1`; health handles strip to `/health`). Every legacy handle adds `header Deprecation "true"` and `header Sunset "Sat, 31 Dec 2026 23:59:59 GMT"` (RFC 8594 / RFC 9110 `Sunset`).
